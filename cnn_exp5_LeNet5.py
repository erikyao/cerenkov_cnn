...
"""
Fifth trial of LeNet5 inspired models
Same model configs with cnn_exp3_LeNet5 except using selu for activations (instead of selu)
"""
...
model_configs = [("InputLayer:9x2001x1",
                    "Conv2D:8-1x49-1x1-{}-valid".format(activation),
                    "MaxPooling2D:1x4-1x4-valid",
                    "Conv2D:8-1x49-1x1-{}-valid".format(activation),
                    "MaxPooling2D:1x4-1x4-valid",
                    "Conv2D:16-1x49-1x1-{}-valid".format(activation),
                    "MaxPooling2D:1x4-1x4-valid",
                    "Conv2D:16-7x7-1x1-{}-valid".format(activation),
                    "MaxPooling2D:2x2-2x2-valid",
                    "Flatten", 
                    "Dense:120-{}".format(activation), 
                    "Dense:84-{}".format(activation),
                    "Dense:10-{}".format(activation),
                    "Dense:1-sigmoid") for activation in ["elu", "LeakyReLU"]]
